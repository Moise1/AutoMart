...
describe("Car Sale Ad Test", () => {
    ...
    it("Should not create car if the status is not \"available\"", (done) => {
        chai
            .request(app)
            .post("/api/v1/car")
            .set("Authorization", `Bearer ${sellerToken}`)
            ...
            .end((err, res) => {
                expect(res.body).to.be.an("object");
                expect(res.body.status).to.be.equal(400);
                expect(res.body.data).to.deep.equal("state must be one of [new, used]");
                done();
            });
    });
    ...
    it("User should get one car sale ad", (done) => {
    ...
    it("User should not get a non-existen car sale ad", (done) => {
        ...
        chai
            ...
            .end((err, res) => {
                ...
                expect(res.body.error).to.deep.equal(`Car sale ad number ${car_id} is not found!`);
                done();
    ...
    it("It should  return available cars", (done) => {
        chai
            .request(app)
    ...
    it("It should  not return available cars", (done) => {
        chai
            .request(app)
            .get("/api/v1/car?status=xxx")
            .set("Authorization", `Bearer ${buyerToken}`)
            .end((err, res) => {
                expect(res.body).to.be.an("object");
                expect(res.body.status).to.deep.equal(404);
                expect(res.body.error).to.deep.equal("Sorry! No results could match your search.");
                done();
    ...
    it("Should not  update the status of a non-existent car sale ad", (done) => {
        chai
            .request(app)
            .post("/api/v1/car")
            .set("Authorization", `Bearer ${sellerToken}`)
    ...
    it("Should not delete a non-existent  car sale ad", (done) => {
        chai
            .request(app)
            .post("/api/v1/car")
            .set("Authorization", `Bearer ${sellerToken}`)
